    ...
    pub fn tokenize(&mut self) -> Vec<Token> {
        while self.pos < self.input.len() {
            ...
            match ch {
                '|' => self.read_pipe_or(),
                '&' => self.read_ampersand(),
                ';' => { self.tokens.push(Token::Semi); self.pos += 1; }
                '!' => { self.tokens.push(Token::Bang); self.pos += 1; }
                '>' => self.read_redirect_out(),
                '<' => self.read_redirect_in(),
                '(' => { self.tokens.push(Token::LeftParen); self.pos += 1; }
                ')' => { self.tokens.push(Token::RightParen); self.pos += 1; }
                '{' => { self.tokens.push(Token::LeftBrace); self.pos += 1; }
                '}' => { self.tokens.push(Token::RightBrace); self.pos += 1; }
                '\'' => self.read_single_quote(),
                '"' => self.read_double_quote(),
                '#' => self.skip_comment(),
                '\n' => { self.tokens.push(Token::Newline); self.pos += 1; }
                '$' => self.read_dollar(),
                '`' => { self.tokens.push(Token::Backtick); self.pos += 1; }
                '\\' => self.read_escape(),
                _ => self.read_word(),
            }
        }
    ...
    fn read_dollar(&mut self) {
        if self.pos + 1 < self.input.len() {
            match self.input[self.pos + 1] {
                ...
                _ if self.input[self.pos + 1].is_alphanumeric() || self.input[self.pos + 1] == '_' => {
...
mod tests {
    ...
    #[test]
    fn test_keywords() {
        let mut lexer = Lexer::new("if then elif else for while do in function switch case end begin");
        let tokens = lexer.tokenize();
        assert_eq!(tokens, vec![
            Token::If,
            Token::Then,
    ...
    fn test_path_with_slashes() {
        ...
        let tokens = lexer.tokenize();
        assert_eq!(tokens, vec![
            Token::Word("/usr/bin/env".to_string()),
            Token::Eof,
        ]);
    }
