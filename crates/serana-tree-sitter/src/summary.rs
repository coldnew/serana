...
fn summarize_with_lang(
    ...
    collect_elisions(tree.tree().root_node(), content, lang_id, options, &mut spans);
...
fn normalize_spans(spans: Vec<ElisionSpan>, content: &str) -> Vec<ElisionSpan> {
    ...
    spans.sort_by(|a, b| a.start_line.cmp(&b.start_line).then(a.end_line.cmp(&b.end_line)));
...
fn is_container_item(kind: &str, lang: LanguageId) -> bool {
    match lang {
        LanguageId::Rust => matches!(kind, "impl_item" | "trait_item" | "enum_item" | "struct_item"),
        LanguageId::JavaScript | LanguageId::TypeScript => {
...
fn is_signature_item(kind: &str, lang: LanguageId) -> bool {
    match lang {
        ...
        LanguageId::JavaScript | LanguageId::TypeScript => {
            matches!(kind, "function_declaration" | "method_definition" | "function_expression" | "arrow_function")
        }
...
"#;
        ...
        let kept: String = result.segments.iter()
            .filter_map(|s| s.text.as_deref())
            .collect::<Vec<_>>()
            .join("\n");
