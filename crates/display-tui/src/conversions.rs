...
pub fn color_from_ratatui(c: ratatui::style::Color) -> Color {
    match c {
...
pub fn style_to_ratatui(s: Style) -> ratatui::style::Style {
    ...
    if let Some(fg) = s.fg {
        style = style.fg(color_to_ratatui(fg));
    }
    if let Some(bg) = s.bg {
...
pub fn style_from_ratatui(s: ratatui::style::Style) -> Style {
    Style {
...
pub fn styled_line_to_ratatui(line: StyledLine) -> ratatui::text::Line<'static> {
    let spans: Vec<ratatui::text::Span<'static>> = line
        .spans
        .into_iter()
        .map(|s| ratatui::text::Span::styled(s.text, style_to_ratatui(s.style)))
        .collect();
...
pub fn key_event_from_crossterm(key: crossterm::event::KeyEvent) -> KeyEvent {
...
pub fn key_event_to_crossterm(key: KeyEvent) -> crossterm::event::KeyEvent {
    let code = match key.code {
