...
/// Run Serana's self-modification verification suite.
pub struct VerifySelfTool;
...
fn verification_to_json(result: VerificationResult) -> Value {
    json!({
...
#[async_trait]
impl Tool for VerifySelfTool {
    fn name(&self) -> &'static str {
        "verify_self"
    }

    fn description(&self) -> &'static str {
        "Run Serana's self-modification verification suite (cargo build, then cargo test --quiet). Input: {}"
    }
    ...
    async fn execute(&self, _input: Value) -> Result<Value> {
        let result = tokio::task::spawn_blocking(|| VerificationSystem::new().verify()).await??;
        Ok(verification_to_json(result))
    }
}
...
mod tests {
    ...
    async fn read_self_reads_own_source() {
        let result = ReadSelfTool
            .execute(json!({ "path": "serana/src/tools/self_evolve.rs" }))
            .await
            .unwrap();
    ...
    #[test]
    fn converts_verification_result_to_json() {
        ...
        let json = verification_to_json(result);

        assert_eq!(json["success"], true);
        assert_eq!(json["tests_passed"], 3);
...
pub fn register_self_evolve_tools(registry: &mut ToolRegistry) {
    ...
    registry.register(Box::new(VerifySelfTool));
