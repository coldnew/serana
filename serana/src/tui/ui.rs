...
fn render_status_group(
    segments: &[super::status_line::StatusSegment],
    app: &App,
    theme: &Theme,
    sep: &Span<'static>,
) -> Vec<Span<'static>> {
...
fn render_status_segment(
    seg: super::status_line::StatusSegment,
    app: &App,
    theme: &Theme,
) -> Option<Span<'static>> {
        ...
        ThinkingLevel => {
            if app.thinking_level != super::app::ThinkingLevel::Off {
                let label = match app.thinking_level {
                    super::app::ThinkingLevel::Low => "think:low",
                    super::app::ThinkingLevel::Medium => "think:med",
...
mod tests {
    ...
    fn test_btw_lines_render_rounded_panel() {
        ...
        let lines = render_btw_lines(&notes, &theme, &crate::tui::symbols::UNICODE, 40);
        let rendered: Vec<String> = lines.iter().map(|line| line.to_string()).collect();
    ...
    fn test_btw_lines_fit_requested_width() {
        ...
        let lines = render_btw_lines(&notes, &theme, &crate::tui::symbols::UNICODE, 24);
        assert!(lines
            .iter()
    ...
    fn test_btw_lines_fit_tiny_width() {
        ...
        let lines = render_btw_lines(&notes, &theme, &crate::tui::symbols::UNICODE, 8);
        assert!(lines
            .iter()
    ...
    fn test_processing_lines_render_bordered_loader() {
        ...
        let lines = render_processing_lines(&pending, 0, &theme, &crate::tui::symbols::UNICODE, 42);
        let rendered: Vec<String> = lines.iter().map(|line| line.to_string()).collect();
        assert!(rendered[0].contains("┌"));
    ...
    fn test_processing_lines_fit_requested_width() {
        ...
        let lines = render_processing_lines(&pending, 0, &theme, &crate::tui::symbols::UNICODE, 18);
        assert!(lines
            .iter()
