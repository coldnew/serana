...
pub fn register(ns: &mut crate::lisp::ns::Namespace) {
    ns.intern_with_doc(
        "register-set",
        Value::Native(prim_register_set),
        "Store VALUE in the register named CHAR.",
    );
    ...
    ns.intern_with_doc(
        "register-get",
        Value::Native(prim_register_get),
        "Return the value stored in register CHAR, or nil.",
    );
    ns.intern_private_with_doc(
        "get-register",
        Value::Native(prim_register_get),
    ...
    ns.intern_with_doc(
        "register-list",
        Value::Native(prim_register_list),
        "Return an alist of all register names to values.",
    );
    ns.intern_private_with_doc(
        "list-registers",
        Value::Native(prim_register_list),
...
fn prim_register_get(args: &[Value]) -> Result<Value, String> {
    let name = extract_string(args, 0)?;
    let ch = name.chars().next().ok_or("register name must be a char")?;
    with_editor_state(|state| match state.registers.get(&ch) {
