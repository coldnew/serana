...
pub fn create_mode(kind: MajorModeKind) -> Box<dyn MajorMode> {
    match kind {
        MajorModeKind::Fundamental => Box::new(fundamental::FundamentalMode),
        MajorModeKind::Text => Box::new(text::TextMode),
        MajorModeKind::Rust => Box::new(rust_mode::RustMode),
        MajorModeKind::Markdown => Box::new(markdown::MarkdownMode),
        MajorModeKind::Python => Box::new(python::PythonMode),
        MajorModeKind::JavaScript => Box::new(javascript::JavaScriptMode),
        MajorModeKind::Go => Box::new(go::GoMode),
        MajorModeKind::Shell => Box::new(shell::ShellMode),
        MajorModeKind::C => Box::new(c::CMode),
        MajorModeKind::Lisp => Box::new(lisp::LispMode),
    }
}
...
mod tests {
    ...
    fn test_text_mode() {
        ...
        assert_eq!(mode.bracket_pairs().len(), 3);
    }
